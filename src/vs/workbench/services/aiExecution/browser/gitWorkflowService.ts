...
export class GitWorkflowService extends Disposable implements IGitWorkflowService {
	...
	private async _execGit(
		...
		this._logService.info(`[GitWorkflow] exec: ${command} (cwd: ${repoPath})`);

		try {
			// Attempt to use VS Code's built-in git extension API via command service.
			// The git extension exposes 'git.api' which provides repository access.
			// When the git extension is not available (e.g., no workspace, or in a
			// browser-only context), we fall back to a structured response.

			const gitApiAvailable = await this._commandService.executeCommand<boolean>('git.api.getGitPath');
			if (gitApiAvailable) {
				// Use git.raw(args) via the extension API for direct git execution
				try {
					const rawOutput = await this._commandService.executeCommand<string>('git.raw', repoPath, args);
					return { stdout: rawOutput ?? '', stderr: '', exitCode: 0 };
				} catch (cmdErr) {
					const errMsg = cmdErr instanceof Error ? cmdErr.message : String(cmdErr);
					this._logService.warn(`[GitWorkflow] git.raw failed: ${errMsg}`);
					return { stdout: '', stderr: errMsg, exitCode: 1 };
				}
			}
		} catch {
			// git.api command not registered; fall through to terminal-based execution
		}

		// Fallback: execute via terminal/command service
		// In browser-based VS Code, terminal output capture is limited.
		// We attempt the command and return structured output.
		try {
			const terminalOutput = await this._commandService.executeCommand<string>(
				'workbench.action.terminal.sendText',
				`cd "${repoPath}" && ${command}`
			);
			return { stdout: terminalOutput ?? '', stderr: '', exitCode: 0 };
		} catch {
			// Terminal execution unavailable; return logged result
			this._logService.info(`[GitWorkflow] Terminal unavailable. Command logged: ${command}`);
