...
export class AIExecutionService extends Disposable implements IAIExecutionService {
        ...
        async requestFileEdit(edit: IAIFileEdit, context?: IAIMutationContext): Promise<void> {
                ...
                try {
                        // ── STEP 5b: Take snapshot before edit if SnapshotRestore strategy ──
                        if (graphNode.rollbackStrategy === RollbackStrategy.SnapshotRestore && graphNode.fileUri) {
                                await this.rollbackEngine.takeSnapshot(graphNode.id, graphNode.fileUri);
                        }
                        ...
                        try {
                                // Capture the original text before applying the edit (for inverse operations)
                                const originalText = model.textEditorModel.getValueInRange({
                                ...
                                // ── STEP 6b: Store inverse operation if InverseEdit strategy ──
                                if (graphNode.rollbackStrategy === RollbackStrategy.InverseEdit) {
                                        const inverseOps = [{
                                                uri: edit.resource.toString(),
                                                range: {
                                                        startLine: edit.rangeStartLineNumber,
                                                        startCol: edit.rangeStartColumn,
                                                        endLine: edit.rangeEndLineNumber,
                                                        endCol: edit.rangeEndColumn,
                                                },
                                                newText: originalText,
                                        }];
                                        this.storageService.store(
                                                `aiExecution.inverseOps.${graphNode.id}`,
